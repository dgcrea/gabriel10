...
export default class MenuScene extends Phaser.Scene {
  ...
  preload(){
    // Reemplaza 'personaje' por tu nombre clave y ajusta el frameWidth y frameHeight
  ...
  create() {
    ...
    const height = this.cameras.main.height;
        ...
        // Texto principal
    const titleStyle = { fontFamily: 'Trebuchet MS', fontSize: '28px', color: '#000', align: 'center' ,backgroundColor: 'rgba(255,255,255,0.9)',    
      wordWrap: { 
        width: 300, 
...
// Botón Jugar (rect + texto)
    ...
    // Mostrar fila de niveles (opcional): 1..MAX_LEVELS con candado si bloqueado
// colocar niveles en 2 filas (1..5 y 6..10)
const cols = 5;
const rows = 2;
...
for (let r = 0; r < rows; r++) {
  for (let c = 0; c < cols; c++) {
    ...
    if (unlocked) {
      ...
      rRect.on('pointerdown', () => {
        this.scene.start('PuzzleScene', { level: level });
      });
  ...
  if (level > MAX_LEVELS) break;
    ...
    this.copa = this.add.sprite(280, 740, 'copagirar').setScale(0.6);;
